...
public final class ExecUtils
{
    ...
    private static Logger LOGGER = LogManager.getLogger(ExecUtils.class);
        ...
        try
        {
            ...
            while( true )
            {
                synchronized( proc )
                {
                    ...
                    {
                        LOGGER.info("Exec stdout:  [" + stdOut.getResult() + "]"); //$NON-NLS-1$
                        LOGGER.info("Exec stderr:  [" + stdErr.getResult() + "]"); //$NON-NLS-1$
                        LOGGER.info("Exec status:  [" + exitStatus + "]"); //$NON-NLS-1$
                        break;
                    }
                    proc.wait();
                }
