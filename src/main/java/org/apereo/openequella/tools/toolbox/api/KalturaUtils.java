...
// TODO - need to abstract out Kaltura code for a cleaner build.
public class KalturaUtils {
  ...
  //		} else {
  //			// Precondition: the check of the file suffix being in the audio or video CSV has already
  // happened.
  //			// Assumes video type
  ...
  //							public void onComplete(Response<MediaEntry> result) {
  //								if(result.error != null) {
  //									LOGGER.info("ERROR from Kaltura MediaService.addContent:  " +
  ...
  //
  //								// upload
  //								UploadUploadTokenBuilder requestBuilder = UploadTokenService.upload(token.getId(),
  // fileToUpload, false)
  //								.setCompletion(new OnCompletion<Response<UploadToken>>() {
  //
  //									@Override
  //									public void onComplete(Response<UploadToken> result) {
  //										if(result.error != null) {
  //											LOGGER.info("ERROR from Kaltura UploadTokenService.upload:  " +
  // result.error.getMessage());
  //											result.error.printStackTrace();
  //											return;
  //										}
  ...
  //										LOGGER.info("{} - Finished uploading resource's content object to Kaltura.  Media ID
  // in Kaltura:  {}.", eItem.getSignature(), entry.getId());
  ...
  //
  //		LOGGER.info("{} - Confirming openEQUELLA resource content object in Kaltura:  ",
  // item.getSignature(), kId);
  //
  ...
  //			public void onComplete(Response<MediaEntry> result) {
  //				if(result.error != null) {
  //					LOGGER.info("{} - ERROR from Kaltura MediaService.get ({}):  {}", item.getSignature(), kId,
  // result.error.getMessage());
  //					result.error.printStackTrace();
  //					return;
  //				}
  ...
  //						throw new RuntimeException(String.format("%s - Hit max retries (%s) when retrieving entry:
  // %s", item.getSignature(), maxTries, kId));
  //					} else {
  //						LOGGER.info("{} - On try: {}, clip not ready (status = {}. waiting {} seconds...",
  // confirmationCounter, item.getSignature(), retrievedEntry.getStatus().getValue(), (sleepInterval
  // / 1000));
  //						try {
  ...
  //				}
  //				else {
  //					LOGGER.info("{} - Confirmed openEQUELLA resource content object in Kaltura looks good.
  // Kaltura Media ID:  {}", item.getSignature(), kId);
