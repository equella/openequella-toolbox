...
public class ReportManager {
  ...
  public void setup(boolean noFiles) {
    ...
    try {
      stdOut =
          String.format(
              "%s/oEQ_checkFiles_%s_%s_all_stats_%s.csv",
              Config.get(Config.CF_OUTPUT_FOLDER),
              Config.get(Config.CF_MODE),
              Config.get(Config.CF_ADOPTER_NAME),
              stats.getStartTimeStr());
      ...
      errOut =
          String.format(
              "%s/oEQ_checkFiles_%s_%s_error_stats_%s.csv",
              Config.get(Config.CF_OUTPUT_FOLDER),
              Config.get(Config.CF_MODE),
              Config.get(Config.CF_ADOPTER_NAME),
              stats.getStartTimeStr());
      ...
      if (noFiles) {
        ...
        errorStatsWriter = new BufferedWriter(new FileWriter(errorStatsFile));
      }
  ...
  public boolean finalizeReporting() {
    try {
      ...
      if (!Check.isEmpty(compareEnabled) && Boolean.valueOf(compareEnabled)) {
        ...
        comparison = CheckFilesUtils.looseCompare(prevErrors, new File(errOut));
        if (comparison == null) {
          ...
          logger.info(
              "Comparison result:  This report and the last report DO NOT have the same missing attachments!");
          logger.info("Resolved attachments from [{}]", comparison.getFirstName());
          for (ResultsRow r : comparison.getOnlyInFirst()) {
            logger.info("- {}", r);
          }
          logger.info("New attachment issues from [{}]", comparison.getSecondName());
          for (ResultsRow r : comparison.getOnlyInSecond()) {
            logger.info("- {}", r);
          }
        } else {
          logger.info(
              "Comparison result:  This report and the last report HAVE the same missing attachments!");
        }
      }
      ...
      if (Config.getCheckFilesEmailModeConfig() == Config.CheckFilesEmailMode.NORMAL) {
      ...
      } else if ((Config.getCheckFilesEmailModeConfig()
              == Config.CheckFilesEmailMode.ONLY_NEW_MISSING_ATTACHMENTS_OR_ERRORS)
          && (comparison != null)
          && !comparison.areReportsEqual()) {
        kickOffEmailHtmlReport("change in missing attachments");
          ...
          && comparison.areReportsEqual()) {
        logger.error(
            "No change in missing attachments, so not sending out the 'only missing attachments' email report.");
      } else if ((Config.getCheckFilesEmailModeConfig()
              == Config.CheckFilesEmailMode.ONLY_NEW_MISSING_ATTACHMENTS_OR_ERRORS)
          && (comparison == null)) {
        logger.error(
            "Email ONLY_NEW_MISSING_ATTACHMENTS_OR_ERRORS specified, but comparison was unable to be built - sending an email anyways!.");
      ...
      logger.error("Unable to finalize reporting due to: {}", e.getMessage(), e);
      return false;
    }
  }
  ...
  private void kickOffEmailHtmlReport(String reportType) {
    ...
    EmailUtils.execute(
        "HTML", Config.get(Config.CF_EMAIL_RECIPIENTS), getReportHeader(), buildHtmlReport());
    if (numOfErrors != fatalErrors.size()) {
      logger.error(
          "Unable to send a '{}' email report due to: {}", reportType, getLastFatalError());
    } else {
  ...
  public boolean failFast() {
    ...
    try {
      // Email if enabled
      if (Config.CheckFilesEmailMode.valueOf(Config.get(Config.CF_EMAIL_MODE))
          != Config.CheckFilesEmailMode.NONE) {
        int numOfErrors = fatalErrors.size();
        EmailUtils.execute(
            "HTML",
            Config.get(Config.CF_EMAIL_RECIPIENTS),
            "CRITICAL FAILURE: " + getReportHeader(),
            buildFailFastReport());
        if (numOfErrors != fatalErrors.size()) {
          logger.error(
              "Unable to send failfast notification email due to: {}", getLastFatalError());
          success = false;
        }
      }
      closeWriters();
  ...
  private void internalWriteln(
      BufferedWriter bw, List alternativeResults, String str, Object... args) throws IOException {
    String msg = str;
  ...
  public void generateCsvStats() throws IOException {
    ...
    dualWriteLn("# Of MISSING Attachments,%d", stats.getNumTotalAttachmentsMissing());
    dualWriteLn("# Of IGNORED Attachments,%d", stats.getNumTotalAttachmentsIgnored());
    ...
    if (!Config.get(Config.CF_MODE).equalsIgnoreCase("REST")) {
      ...
      dualWriteLn("Total duration of queries (ms),%d", stats.getTotalDurationOfQueriesRan());
      dualWriteLn("Average duration of queries (ms),%d", stats.getAverageDurationOfQueriesRan());
    }
  }
  ...
  public void displayStats() throws IOException {
    ...
    logger.info("# Of MISSING Attachments: {}", stats.getNumTotalAttachmentsMissing());
    logger.info("# Of IGNORED Attachments: {}", stats.getNumTotalAttachmentsIgnored());
    ...
    if (!Config.get(Config.CF_MODE).equalsIgnoreCase("REST")) {
      ...
      logger.info("Total duration of queries (ms): {}", stats.getTotalDurationOfQueriesRan());
      logger.info("Average duration of queries (ms): {}", stats.getAverageDurationOfQueriesRan());
    }
  }

  public String getReportHeader() {
    return String.format(
        "oEQ-Toolbox:CheckFiles v%s - %s Report", Config.VERSION, stats.getStartTimeStr());
  }
  ...
  public String buildHtmlReport() {
    ...
    // Current run
    append(sb, "<div style='color:blue'><b>Details of current run:</b></div><ul>");
    append(sb, "<li><b>Report Duration (seconds)</b> - %d</li>", stats.getDuration());
    ...
    append(sb, "<li><b># Of Items Checked</b> - %d</li>", stats.getNumTotalItems());
    append(sb, "<li><b># Of Attachments Checked</b> - %d</li>", stats.getNumTotalAttachments());
    append(
        sb, "<li><b># Of Attachments Missing</b> - %d</li>", stats.getNumTotalAttachmentsMissing());
    append(sb, "</ul>");
    ...
    // Comparison
    append(sb, "<div style='color:blue'><b>Details of comparison report:</b></div>");
    if (comparison != null) {
      append(
          sb, "<div>Comparison of <i>missing</i> attachments from previous run to current run: ");
      if ((comparison != null) && !comparison.areReportsEqual()) {
        append(sb, "<span style='color:red'><b>CHANGED</b></span></div>");

        append(
            sb, "<br/><div><i><b>Placement legend: </b> %s</i></div><br/>", ResultsRow.getHeader());
        ...
        append(sb, "<span style='color:blue'><b>NO CHANGE</b></span></div>");
      }
      append(
          sb, "<div><b>Comparison file for previous run:</b> %s</div>", comparison.getFirstName());
      append(
          sb, "<div><b>Comparison file for current run:</b> %s</div>", comparison.getSecondName());
    } else {
      append(sb, "<div>No comparison specified, or a comparison wasn't able to be made.</div>");
    }
    append(
        sb,
  ...
  public String buildFailFastReport() {
    ...
    append(sb, "<div>Please check the Toolbox logs for more details...</div>");
    append(
        sb,
    ...
    try {
      ...
      if (Config.CheckFilesType.valueOf(Config.get(Config.CF_MODE)) == Config.CheckFilesType.REST) {
        ...
        if (!Check.isEmpty(Config.get(Config.CF_FILTER_BY_INSTITUTION))) {
          dualWriteLn(
              "Filter By Institution Shortname,%s", Config.get(Config.CF_FILTER_BY_INSTITUTION));
        } else {
          dualWriteLn("Filter By Institution Shortname,%s", notSpecified);
        }
      ...
      dualWriteLn("Report Timestamp,%s", ReportManager.getInstance().getReportTimestamp());

      dualWriteLn(ResultsRow.getHeader());
      return true;
    } catch (IOException e) {
      logger.error("Unable to setup reporting details due to: {}", e.getMessage(), e);
      return false;
    }
  }
